...
class CausalMetric():
    ...
    def evaluate(self, img_batch, exp_batch, batch_size):
        ...
        salient_order = np.flip(np.argsort(
            exp_batch.reshape(-1, HW), axis=1), axis=-1)
        ...
        for i in tqdm(range(n_steps+1), desc=caption + 'pixels'):
            ...
            if i < n_steps:
                for rr in r:
                    start.cpu().numpy().reshape(n_samples, 3, HW)[rr, :, coords.reshape(n_samples, 3, n_steps)[
                        rr]] = finish.cpu().numpy().reshape(n_samples, 3, HW)[rr, :, coords.reshape(n_samples, 3, n_steps)[rr]]
